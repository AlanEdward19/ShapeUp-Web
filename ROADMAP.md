...
Lado 1 do marketplace. Depende de verificação de credencial funcionando. **[Cross-repo]** — ver também `ShapeUpApi/ROADMAP.md`.
...
- Apps Android/iOS
- Sync (mutation queue com `operation_id` para idempotência — crítico em treino, ShapeCoins, pagamentos, check-ins) — contraparte backend do mecanismo em `ShapeUpApi/ROADMAP.md`
- Wearable foundation (HealthKit, Health Connect — Garmin/Fitbit/Whoop/Oura ficam para depois)

## Fase 7 — Social

⚠️ Social safety é pré-requisito, não segue-junto. **[Cross-repo]** — ver também `ShapeUpApi/ROADMAP.md`.
