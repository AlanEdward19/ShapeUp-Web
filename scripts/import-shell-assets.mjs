...
if (!source) throw new Error('Provide the design export directory.');
const names = {
...
await mkdir('src/pages/shell-assets/styles', { recursive: true });
await writeFile('design/screens/DESIGN.md', await readFile(path.join(source, 'warm_oxide_athletic/DESIGN.md')));
const manifest = {};
for (const folder of await readdir(source)) {
  ...
  await writeFile(`design/screens/${name}.html`, html);
  ...
  css.walkRules(rule => {
    ...
    rule.selector = selectorParser(selectors => {
      selectors.walkTags(tag => { if (['html', 'body'].includes(tag.value)) tag.replaceWith(selectorParser.className({ value: 'shell-body' })); });
      selectors.walkPseudos(pseudo => { if (pseudo.value === ':root') pseudo.value = ':host'; });
    }).processSync(rule.selector);
  });
  await writeFile(`src/pages/shell-assets/styles/${name}.css`, css.toString());
...
await writeFile('src/pages/shell-assets/manifest.json', JSON.stringify(manifest, null, 2));
