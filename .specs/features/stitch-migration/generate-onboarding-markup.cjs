/**
 * One-off: onboarding.html → static JSX fragment (checked into Dashboard/markup/).
 */
const fs = require('fs');
...
const outDir = path.join(__dirname, '../../../src/pages/Dashboard/markup');
...
function nodeToJsx(node, indent = '      ') {
  ...
  for (const attr of node.attributes) {
    ...
    if (attr.name === 'data-source-onclick') {
      props['data-source-onclick'] = attr.value;
      continue;
    }
...
const html = fs.readFileSync(path.join(templatesDir, 'onboarding.html'), 'utf8');
...
fs.writeFileSync(path.join(outDir, 'OnboardingStaticMarkup.tsx'), file);
console.log('wrote OnboardingStaticMarkup.tsx');
