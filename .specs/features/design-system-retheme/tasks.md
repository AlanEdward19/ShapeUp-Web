...
| Auth-blocked routes (`/dashboard`, sidebar on workspace) | unit (fallback) | When login redirect blocks PNG capture: consumer files use `var(--…)` and pass retired-hex hygiene; public routes still require PNGs | `src/styles/__tests__/designSystemRethemeVisualEvidence.test.js` | `npm test` |
